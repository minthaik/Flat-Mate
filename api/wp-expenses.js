...
export default async function handler(req, res) {
  ...
  const endpoint = `${base}/wp-json/flatmate/v1/expenses`;
  ...
  try {
    ...
    if (req.method === "POST") {
      const body = req.body || {};
      if (!body.houseId || typeof body.amount === "undefined") {
        return res.status(400).json({ error: "houseId and amount required" });
      }
    ...
    return res.status(500).json({ error: "WP expenses proxy failed", detail: err?.message || "Unknown error" });
  }
}
